...
//Disable all LEDs
  ...
  ledc_set_duty(LEDC_MODE, LEDC_CHANNEL, 5000);
  ledc_update_duty(LEDC_MODE, LEDC_CHANNEL);
  vTaskDelay(1000 / portTICK_PERIOD_MS);
...
// ==================== END GPIO-TEST =========================== 
  ...
  switch (wakeSelector()){   
      case CMD_OPEN:  strcpy(cfg.cmd , "systemctl start jetapp-jvrvg&&exit");  printf("ssh cmd stop\n"); break;
      case CMD_CLOSE: strcpy(cfg.cmd, "systemctl stop jetapp-jvrvg&&exit");  printf("ssh cmd stop\n");   break;
      default:     
        printf("UNKNOWN!\n");
        ...
        if (1 == gpio_get_level(GPIO_NUM_2)){
          ...
          //while (vTaskDelay(10000 / portTICK_PERIOD_MS);  //idle forever

        }
        vTaskDelay(10000 / portTICK_PERIOD_MS);  //idle forever
        enterDeepsleep();
      break;
  }
